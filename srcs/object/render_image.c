...
t_cam	*init_cam_struct(t_cam_info *info, t_vector *origin)
{
	...
	new->lower_left_corner = subtract_two_vectors(subtract_two_vectors
			(new->origin, vec_mul_scalar_apply(new->horizontal, 0.5)),
			subtract_two_vectors(vec_mul_scalar_apply(new->vertical, 0.5),
				vec_create(0, 0, info->focal_lenght)));
	return (new);
}
